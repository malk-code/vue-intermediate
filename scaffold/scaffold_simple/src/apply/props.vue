<template>
	<div class="main">
		<h3>导入自定义组件:</h3>
		<!--实现了绑定入参，绑定事件，事件抛出值-->
		<VButton :button="button.title1" @button-click="buttonClick($event)"></VButton>
		<VButton v-bind:button="button.title2" v-on:button-click="buttonClick($event)"></VButton>
    		<!--13. 组件props：若直接传入不能使用冒号；冒号是绑定传入：显示和数据是同步的。它们在组件内都能通过props取到值。props支持多参，支持传入对象，可为传入对象的属性添加验证-->
		<VButton button="直接传入值" v-on:button-click="buttonClick($event)"></VButton>
		<!--弱类型，可传入其他类型，如44: 支持多参-->
		<VButton button="false" tmp="和上面的控件是同一个组件" v-on:button-click="buttonClick($event)"></VButton>
		<!--6. props：支持多参，可做参数验证，尤其是开放为公共组件时尤其有用。自定义组件在绑定时，v-bind可省略，直接使用 :设置的props，若为点击事件，可直接使用@自定义的事件名称-->
		<h3>props 多参验证:</h3>
		<VProps :propA="props.propA"
				:propB="props.propB"
				:propC="props.propC"
				:propD="props.propD"
				:propE="props.propE"
